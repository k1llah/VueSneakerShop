...
<template>
	<div class="flex flex-col justify-between p-4 cursor-pointer md:min-w-[550px] sm: min-w-full md:max-w-[350px] bg-cover min-h-[350px]"
	...
	>
      ...
      <p v-for="(hashtag, index) in hashtags" :key="index" class="text-black text-xs font-medium bg-[#10b981] p-2 rounded-lg hover:bg-[#eab308] transition-all duration-200 hover:transition-all hover:duration-200">{{ hashtag }}</p>
		...
		<h1 class="md:text-3xl font-bold sm:text-2xl">{{ props.titleCover }}</h1>
		<p class="md:text-lg font-light sm:text-base">{{ props.shortDescription }}</p>
	</div>
	</div>
</template>